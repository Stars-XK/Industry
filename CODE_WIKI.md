...
*   **`src/views/`**: 承载所有的业务页面。
    ...
    *   `analytics/`: 产销差漏损报表、夜间最小流量 (MNF)、计费对账、预测分析。
...
*   **`src/layout/`**: 系统的基础导航框架（侧边栏、顶栏），不包含独立大屏 `/dashboard`。
...
### 4.2 多维统计与数据分析 (Analytics)
该模块作为数据价值变现核心，承担产销差计算、夜间最小流量挖掘及大客户营收计费。
*   **前端对应**: `views/analytics/nrw.vue` (产销差漏损)、`mnf.vue` (夜间最小流量)、`billing.vue` (营收计费)。
*   **后端对应**: `data-center` (`NrwController`, `BillingController`)。
*   **业务逻辑**:
    *   **产销差拆解**: 按照国际水协 (IWA) 标准，将“总供水量”通过桑基图结构化拆解为合法用水、真实漏损、表观漏损等，支持多维度同环比分析。
    *   **真实的抄表计费引擎**: 提取大户上期和当期的抄表底数，相减计算实际用水量，关联阶梯费率完成出账。
    *   **工业级防御**: 检测到底数倒转时抛出表计异常；大户档案或费率已被引用时，系统执行强关联拦截，仅允许停用。
