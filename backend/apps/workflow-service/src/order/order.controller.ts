...
export class OrderController {
  ...
  async getOrders() {
    const query = `
      ...
      LEFT JOIN ast_device d ON o.device_id = d.id
  ...
  async createOrder(@Body() body: any, @Request() req: any) {
    const { order_type, alarm_id, device_id, title, description, priority, handler_id, gis_coord } = body;
    ...
    await this.dataSource.query(
      `INSERT INTO wf_work_order 
       (order_sn, order_type, alarm_id, device_id, title, description, priority, status, creator_id, handler_id, gis_coord) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [order_sn, order_type, alarm_id || null, device_id || null, title, description, priority || 2, 10, req.user.userId, handler_id || null, gis_coord || null]
    );
    return { success: true };
  }
