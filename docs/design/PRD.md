...
- 补充**用户**，补充**部门**，补充**分区**。
- 补充**用户水卡信息**（建立物理分区与商业收费的纽带）。
- 补充**站点** (水厂、加压泵站、二供泵房、管网监测点)。
- 补充**设备** (每个站点下的设备列表)。
- 补充**测点** (每个设备输出的数据类型，如瞬时流量、累计流量、压力等)。

### 2.2 实时数据采集与清洗计算 (TDengine)
- **数据抓取**：补充实时数据数据源，然后定时抓取数据存放 `tgen` (TDengine) 中。
- **清洗与计算**：在 `tgen` 中清洗数据，计算出设备 **5分钟、1小时** 的数据。
- **状态监控**：同时还需要有一个设备**最新数据**用于前端监控与大屏展示。

### 2.3 供水端指标提取与流转
- **日供水量提取**：从设备累计流量中切割出 **设备日用量**，然后根据分区下的设备列表，找到日累计数据，汇总计算出 **分区日供水量**、**分区月供水量**。
- **夜间最小流量 (MNF)**：利用分区五分钟流量计计算出的瞬时数据量，提取得到 **2:00-4:00 点的分区夜间最小流量**。

### 2.4 营收端数据流转与售水量计算
- **用户用水量计算**：补充营收数据来源，根据营收数据类型（日用、月用或累计），计算出每个用户每日的 **日用水量** 和 **月用水量**。
- **分区售水量汇总**：根据分区下关联的用户水卡信息，将单个用户的用量进行空间汇总，计算出 **分区日用量(售水)** 和 **分区月用量(售水)**。

### 2.5 产销差计算与全域展现 (NRW & Global Display)
- **产销差 (NRW) 计算**：通过 `分区供水 - 分区售水`，计算出 **产销差量**，进行全域展示。
