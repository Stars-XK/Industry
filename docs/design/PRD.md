...
- **数据清洗与标准化**：依托**独立的时序数据库（如 TDengine 等）自带的流计算引擎**进行数据清洗，将海量原始数据**标准化聚合为 1分钟、5分钟、1小时** 等多时间维度的数据。
  - **异构设备表计数据标准化（累积量/瞬时量自动换算）**：针对底层设备上报类型的复杂性（有的只报累积量，有的报瞬时量，有的报日用量），系统提供内置的数据换算算子。对于只提供“累积量”的营收总表或水表，流计算引擎自动通过 `(当前累积量 - 上一时刻累积量) / 时间差` 转换为实时的“瞬时量”与“时/日区间用量”。当水表发生“翻转（清零）”或更换新表时，系统能自动识别累积量异常跳变并进行接续补偿，确保最终向上层提供的都是标准化的“区间差值用量”与“瞬时流速”。
