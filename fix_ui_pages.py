...
def fix_css(content):
    # 1. Remove .industrial-form wrapper and the ending brace if it wraps everything
    if '.industrial-form' in content:
        # Just remove '.industrial-form {' and the corresponding '}'
        content = re.sub(r'\.industrial-form\s*\{', '', content)
        # It's hard to find the exact closing brace, but we can just let style be
        # Also remove dangling !important
    ...
    # Replace any other rgba(0,216,255, x) with primary light
    content = re.sub(r'rgba\(0,\s*216,\s*255,\s*0\.\d+\)', 'var(--el-color-primary-light-8)', content)
    
    # 3. Ensure layout classes
    # .box-card -> flex layout
    ...
    # Fix .app-container to flex column
    ...
    # Replace transparent backgrounds on inputs/tables
